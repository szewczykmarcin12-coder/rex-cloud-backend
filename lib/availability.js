...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST' && req.query && req.query.action === 'request-bulk') {
      ...
      if (items.length > 62) return res.status(400).json({ success: false, error: 'Za dużo pozycji w jednym zgłoszeniu.' });
      ...
      items.forEach((it, i) => {
        ...
        przygotowane.push({ date: it.date, type: it.type, startTime, endTime, note: String(it.note || '').slice(0, 500) });
      });
      if (bledy.length) return res.status(400).json({ success: false, error: `Popraw pozycje: ${bledy.slice(0, 5).join('; ')}${bledy.length > 5 ? '…' : ''}` });
      const konta = (await kv.get('accounts:list')) || [];
      const konto = konta.find((k) => k.id === accountId);
      if (!konto) return res.status(404).json({ success: false, error: 'Nie znaleziono konta.' });
      const daty = new Set(przygotowane.map((x) => x.date));
      const reqs = (await readReqs()).filter((r) => !(r.accountId === accountId && daty.has(r.date)));
      const teraz = Date.now();
      const nowe = przygotowane.map((x, i) => ({
        ...
        note: x.note, status: 'pending', managerName: null, managerNote: '', decidedAt: null, createdAt: teraz + i,
      }));
      await kv.set(REQS_KEY, [...nowe, ...reqs]);
      await audit({ ...aktor(sesja), action: 'availability.request-month', target: `${konto.login} ${[...daty].sort()[0]}..${[...daty].sort().pop()}`, after: { dni: nowe.length } });
