...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ── zbiorcza dyspozycja na cały miesiąc (Employee Hub) ──
    // body: { items: [{ date, type, startTime, endTime, note }] } — każda data zastępuje poprzedni wpis tej osoby
    if (req.method === 'POST' && req.query && req.query.action === 'request-bulk') {
      const items = Array.isArray((req.body || {}).items) ? req.body.items : [];
      if (!items.length) return res.status(400).json({ success: false, error: 'Brak dni do zgłoszenia.' });
      if (items.length > (sesja.role === 'pracownik' ? 62 : 4000)) return res.status(400).json({ success: false, error: 'Za dużo pozycji w jednym zgłoszeniu.' });
      const manager = sesja.role !== 'pracownik';
      const accountId = sesja.role === 'pracownik' ? sesja.accountId : ((req.body || {}).accountId || null);
      const perItemAcc = manager && items.every((it) => it && it.accountId);
      if (!accountId && !perItemAcc) return res.status(400).json({ success: false, error: 'Wskaż pracownika.' });
      const autoApprove = manager && !!(req.body || {}).autoApprove;
      let okno = null;
      if (sesja.role === 'pracownik') {
        okno = await stanOknaDyspo();
        const [ry, rm] = okno.targetMonth.split('-').map(Number);
        const nazwaMies = new Intl.DateTimeFormat('pl-PL', { month: 'long', year: 'numeric' }).format(new Date(Date.UTC(ry, rm - 1, 1)));
        if (!okno.otwarte) return res.status(403).json({ success: false, error: `Okno dyspozycji na ${nazwaMies} jest zamknięte (termin: 20. dzień miesiąca). Otworzyć może je wyłącznie ASM.` });
        if (items.some((it) => String(it.date || '').slice(0, 7) !== okno.targetMonth)) return res.status(400).json({ success: false, error: `Dyspozycje przyjmujemy wyłącznie na ${nazwaMies}.` });
      }
      const bledy = [];
      const przygotowane = [];
      items.forEach((it, i) => {
        if (!dOk(it.date)) return bledy.push(`${i + 1}: nieprawidłowa data`);
        if (!TYPY_DYSPO.includes(it.type)) return bledy.push(`${it.date}: nieznany typ`);
        const startTime = ['from_time', 'specific_shift'].includes(it.type) ? it.startTime : null;
        const endTime = ['until_time', 'specific_shift'].includes(it.type) ? it.endTime : null;
        if ((startTime && !czasOk(startTime)) || (endTime && !czasOk(endTime))) return bledy.push(`${it.date}: nieprawidłowa godzina`);
        if (['from_time'].includes(it.type) && !startTime) return bledy.push(`${it.date}: podaj godzinę „od"`);
        if (['until_time'].includes(it.type) && !endTime) return bledy.push(`${it.date}: podaj godzinę „do"`);
        if (it.type === 'specific_shift' && !(startTime && endTime)) return bledy.push(`${it.date}: podaj początek i koniec`);
        przygotowane.push({ date: it.date, type: it.type, startTime, endTime, note: String(it.note || '').slice(0, 500), accountId: perItemAcc ? String(it.accountId) : accountId });
      });
      if (bledy.length) return res.status(400).json({ success: false, error: `Popraw pozycje: ${bledy.slice(0, 5).join('; ')}${bledy.length > 5 ? '…' : ''}` });
      const konta = (await kv.get('accounts:list')) || [];
      const kontoMap = new Map(konta.map((k) => [k.id, k]));
      const brakKonta = przygotowane.find((x) => !kontoMap.has(x.accountId));
      if (brakKonta) return res.status(404).json({ success: false, error: `Nie znaleziono konta (${brakKonta.accountId}).` });
      const klucze = new Set(przygotowane.map((x) => `${x.accountId}|${x.date}`));
      const reqs = (await readReqs()).filter((r) => !klucze.has(`${r.accountId}|${r.date}`));
      const teraz = Date.now();
      const nowe = przygotowane.map((x, i) => { const konto = kontoMap.get(x.accountId); return {
        id: 'dy' + (teraz + i) + Math.random().toString(36).slice(2, 6),
        accountId: x.accountId, name: konto.name, login: konto.login,
        date: x.date, type: x.type, startTime: x.startTime, endTime: x.endTime, recurrence: 'once', repeatUntil: null,
        note: x.note, status: autoApprove ? 'approved' : 'pending', managerName: autoApprove ? sesja.name : null, managerNote: autoApprove ? 'import' : '', decidedAt: autoApprove ? new Date().toISOString() : null, createdAt: teraz + i,
      }; });
      await kv.set(REQS_KEY, [...nowe, ...reqs]);
      const daty = [...new Set(nowe.map((x) => x.date))].sort();
      await audit({ ...aktor(sesja), action: perItemAcc ? 'availability.import' : 'availability.request-month', target: `${perItemAcc ? new Set(nowe.map((x) => x.accountId)).size + ' osób' : kontoMap.get(accountId).login} ${daty[0]}..${daty[daty.length - 1]}`, after: { dni: nowe.length, autoApprove } });
      return res.json({ success: true, dni: nowe.length, requests: await zKonfliktami(nowe) });
    }

    // ── decyzja zbiorcza managera (np. cały miesiąc jednej osoby) ──
    if (req.method === 'POST' && req.query && req.query.action === 'decide-bulk') {
      if (sesja.role === 'pracownik') return res.status(403).json({ success: false, error: 'Decyzja wymaga uprawnień kierownika.' });
      const { ids, status, managerNote } = req.body || {};
      if (!Array.isArray(ids) || !ids.length) return res.status(400).json({ success: false, error: 'Wskaż zgłoszenia.' });
      if (!['approved', 'rejected'].includes(status)) return res.status(400).json({ success: false, error: 'Decyzja: approved albo rejected.' });
      const zbior = new Set(ids.map(String));
      const reqs = await readReqs();
      const teraz = new Date().toISOString(); let n = 0; const osoby = new Set();
      reqs.forEach((rq) => { if (zbior.has(rq.id)) { rq.status = status; rq.managerName = sesja.name; rq.managerNote = String(managerNote || '').slice(0, 300); rq.decidedAt = teraz; n++; osoby.add(rq.login); } });
      await kv.set(REQS_KEY, reqs);
      await audit({ ...aktor(sesja), action: `availability.${status === 'approved' ? 'approve' : 'reject'}-bulk`, target: [...osoby].join(', '), after: { zgloszen: n } });
      const zK = await zKonfliktami(reqs.filter((rq) => zbior.has(rq.id)));
      return res.json({ success: true, zmienione: n, requests: zK });
    }
