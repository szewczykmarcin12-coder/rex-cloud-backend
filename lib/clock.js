...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET' && action !== 'hub-state') {
      if (!(await requireRole(req, res, ['asm', 'kierownik']))) return;
      const d = (req.query && req.query.date) || opDay();
      const events = (await kv.get(KEY(d))) || [];
    ...
    if (req.method === 'POST' && action === 'terminal-add') {
      ...
      await audit({ ...aktor(sesjaT), action: 'terminal.add', target: tid });
    ...
    if (req.method === 'POST' && action === 'terminal-toggle') {
      ...
      await kv.set(TERMS_KEY, terms);
      await audit({ ...aktor(sesjaT), action: t.active ? 'terminal.activate' : 'terminal.deactivate', target: t.id });
      return res.json({ success: true, terminals: terms });
    }
    if (req.method === 'POST' && action === 'terminal-del') {
      ...
      await kv.set(TERMS_KEY, terms);
      await audit({ ...aktor(sesjaT), action: 'terminal.delete', target: String((req.body || {}).id || '').trim() });
      return res.json({ success: true, terminals: terms });
    }

    // ══ ORDO Employee Hub — rejestracja czasu przez SESJĘ pracownika (bez terminala) ══
    // Ten sam event store i maszyna stanów co terminal; method='app', terminal='EMPLOYEE-HUB'.
    if (req.method === 'GET' && action === 'hub-state') {
      const sesja = await requireRole(req, res, ['pracownik']);
      if (!sesja) return;
      const d = opDay();
      const events = (await kv.get(KEY(d))) || [];
      const moje = events.filter((e) => e.accountId === sesja.accountId);
      let planned = null;
      try {
        const pub = await kv.get(`sched:pub:${d.slice(0, 7)}`);
        const zm = ((pub && pub.shifts) || []).find((x) => x.date === d && x.accountId === sesja.accountId && x.rola !== 'instruktor');
        if (zm) planned = { start: zm.start, end: zm.end, station: zm.station || null };
      } catch {}
      return res.json({ success: true, date: d, state: stanKonta(events, sesja.accountId), events: moje, planned });
    }

    if (req.method === 'POST' && action === 'hub-event') {
      const sesja = await requireRole(req, res, ['pracownik']);
      if (!sesja) return;
      const { action: typ, breakType, clientEventId } = req.body || {};
      ...
      if (clientEventId && events.some((e) => e.cid === clientEventId)) {
        return res.json({ success: true, state: stanKonta(events, sesja.accountId), duplicate: true });
      }
      const st = stanKonta(events, sesja.accountId);
      ...
      events.push({
        ...
        accountId: sesja.accountId, login: sesja.login || null, name: sesja.name || null,
        type: typ, paid: typ === 'break_start' ? breakType === 'paid' : undefined,
        method: 'app', terminal: 'EMPLOYEE-HUB',
        at: at.getTime(), atISO: at.toISOString(), serverReceivedAt: at.toISOString(), opDay: d,
      });
      await kv.set(KEY(d), events);
      const mojeTeraz = events.filter((e) => e.accountId === sesja.accountId);
      return res.json({ success: true, state: stanKonta(events, sesja.accountId), occurredAt: at.toISOString(), events: mojeTeraz });
    }
