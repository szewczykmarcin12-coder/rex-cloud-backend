...
// pełne imiona i nazwiska z KONT (alias z grafiku zostaje kluczem technicznym)
async function slownikKont() {
  ...
  konta.forEach((a) => { const k = normalizeName(a.name || ''); if (k && !poAlias.has(k)) poAlias.set(k, a); });
  return { konta, poAlias };
}
function wzbogac(swaps, poAlias) {
  const disp = (n) => { const a = n && poAlias.get(normalizeName(n)); return (a && a.name) || n; };
  return swaps.map((x) => ({
    ...x,
    requesterDisplay: disp(x.requester),
    volunteersDisplay: (x.volunteers || []).map((v) => ({ alias: v, display: disp(v) })),
    approvedVolunteerDisplay: x.approvedVolunteer ? disp(x.approvedVolunteer) : null,
  }));
}
...
async function reassign(sw, kto, ktoId) {
  const ym = (sw.date || '').slice(0, 7);
  const bucket = await kv.get(monthKey(ym));
  if (!bucket || !Array.isArray(bucket.shifts)) return null;
  const konta = (await kv.get('accounts:list')) || [];
  // tożsamość przejmującego: najpierw KONTO z sesji zgłoszenia, alias tylko awaryjnie
  const noweKonto = (ktoId && konta.find((a) => a.id === ktoId))
    || konta.find((a) => [a.grafikName, ...(a.aliasy || []), a.name].filter(Boolean).some((n) => normalizeName(n) === normalizeName(kto)));
  let przed = null, po = null;
  bucket.shifts = bucket.shifts.map((s) => {
    const tenSam = sw.requesterAccountId ? s.accountId === sw.requesterAccountId : normalizeName(s.name) === normalizeName(sw.requester);
    if (!przed && tenSam && s.date === sw.date &&
        s.station === sw.shift.station && s.start === sw.shift.start && s.end === sw.shift.end) {
      przed = s;
      po = { ...s, name: (noweKonto && noweKonto.grafikName) || kto };
...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      const { poAlias } = await slownikKont();
      return res.json({ success: true, swaps: wzbogac(await readSwaps(), poAlias) });
    }
    ...
    if (req.method === 'POST') {
      ...
      const { poAlias } = await slownikKont();
      const kontoReq = sesja.role === 'pracownik'
        ? { id: sesja.accountId }
        : poAlias.get(normalizeName(requester)) || null;
      const swap = {
        id: 'sw' + Date.now() + Math.random().toString(36).slice(2, 6),
        requester, requesterAccountId: (kontoReq && kontoReq.id) || null,   // tożsamość = konto, nie alias
        date: shift.date, shift, note: note || '',
        status: 'open', volunteers: [], volunteerIds: {}, approvedVolunteer: null, createdAt: Date.now(),
      };
      swaps.unshift(swap);
    ...
    if (req.method === 'PUT') {
      ...
      if (action === 'volunteer') {
        ...
        if (sesja.role === 'pracownik' && sw.requesterAccountId && sw.requesterAccountId === sesja.accountId) return res.status(400).json({ success: false, error: 'Nie można zgłosić się do własnej zmiany' });
        ...
        if (sesja.role === 'pracownik') sw.volunteerIds = { ...(sw.volunteerIds || {}), [normalizeName(name)]: sesja.accountId };
      } else if (action === 'unvolunteer') {
        sw.volunteers = sw.volunteers.filter((v) => normalizeName(v) !== normalizeName(name));
      } else if (action === 'cancel') {
        const mojaProsba = sw.requesterAccountId ? sw.requesterAccountId === sesja.accountId : normalizeName(sw.requester) === normalizeName(wlasna);
        if (sesja.role === 'pracownik' && !mojaProsba) return res.status(403).json({ success: false, error: 'Można anulować tylko własną prośbę.' });
        if (sw.status === 'open') sw.status = 'cancelled';
      } else if (action === 'reject') {
        sw.status = 'rejected';
        await audit({ ...aktor(sesja), action: 'swap.reject', target: sw.id, before: { requester: sw.requester, shift: sw.shift } });
      } else if (action === 'approve') {
        const kto = volunteer || sw.volunteers[0];
        if (!kto) return res.status(400).json({ success: false, error: 'Brak osoby zgłoszonej' });
        const ktoId = (sw.volunteerIds || {})[normalizeName(kto)] || null;
        const wynik = await reassign(sw, kto, ktoId);
        ...
        await audit({ ...aktor(sesja), action: 'swap.approve', target: sw.id, before: wynik.przed, after: wynik.po, reason: sw.note || null });
      } else {
