  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (req.body && (req.body.action === 'link' || req.body.action === 'unlink')) {
        ...
        audit({ actor: s.name, role: 'asm', action: req.body.action === 'link' ? 'role.link' : 'role.unlink', target: cel, after: req.body.action === 'link' ? { rola: roles[cel] } : null });
      ...
      if (login != null && password != null) {
        ...
        if (asmLogin && kandydat.toLowerCase() === String(asmLogin).toLowerCase() && (await checkStored('asm:pass', String(password)))) {
          ...
          audit({ actor: kandydat, role: 'asm', action: 'auth.login', target: 'panel', after: { ip } });
        ...
        if (konto && rola) {
          ...
          if (w.ok) {
            ...
            audit({ actor: cel, role: rola, action: 'auth.login', target: 'panel', after: { ip } });
            return res.json({ success: true, role: rola, userName: (konto.name || cel).trim(), login: cel, token });
          }
          audit({ actor: cel, role: null, action: 'auth.login-failed', target: 'panel', after: { ip } });
    ...
    if (req.method === 'PUT') {
      ...
      if (body.newPin != null) {
        ...
        audit({ actor: s.name, role: 'asm', action: 'auth.change-manager-pin', target: 'admin:pin' });
      ...
      if (body.newPassword != null || body.newLogin != null) {
        ...
        audit({ actor: s.name, role: 'asm', action: 'auth.change-asm-credentials', target: 'asm' });
        return res.json({ success: true });
      }
