...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      return res.json({ success: true, asmLogin, linked });
    }
    ...
    if (req.method === 'POST') {
      ...
      if (login != null && password != null) {
        ...
        if (roles[cel]) {
          const konta = (await kv.get('accounts:list')) || [];
          const konto = konta.find((a) => String(a.login || '').toUpperCase() === cel);
          if (konto && konto.hasloHash === sha(String(password))) {
            ...
            return res.json({ success: true, role: roles[cel], userName: (konto.name || cel).trim(), login: cel });
          }
        }
