...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST' && req.query && req.query.action === 'add') {
      ...
      audit({ ...aktor(sesjaMod), action: 'schedule.add', target: `${ym}/${zmiana.sid}`, after: zmiana });
      return res.json({ success: true, shift: zmiana, version: wersja });
    }
    ...
    if (req.method === 'POST' && req.query && req.query.action === 'update') {
      ...
      m.shifts[idx] = { ...st, sid: st.sid || newSid(), name: osoba, station: (nowe.station || st.station || '').toUpperCase(), start: nStart, end: nEnd, hours: godz, ...(idKonta ? { accountId: idKonta } : {}) };
      const wersja = await saveMonth(ym, m);
      audit({ ...aktor(sesjaMod), action: 'schedule.update', target: `${ym}/${m.shifts[idx].sid}`, before: st, after: m.shifts[idx] });
      return res.json({ success: true, przed: { date, name: st.name, start, end }, shift: m.shifts[idx], version: wersja });
    }
    ...
    if (req.method === 'POST' && req.query && req.query.action === 'remove') {
      ...
      usuwane.forEach((s) => audit({ ...aktor(sesjaMod), action: 'schedule.remove', target: `${ym}/${s.sid || 'legacy'}`, before: s }));
    ...
    if (req.method === 'PUT') {
      ...
      audit({ ...aktor(sesjaMod), action: 'schedule.import', target: ym, before: { zmian: (stary && (stary.shifts || []).length) || 0 }, after: { zmian: data.shifts.length } });
      const idx = await readIndex();
    ...
    if (req.method === 'DELETE') {
      ...
      if (month) {
        ...
        audit({ ...aktor(sesjaMod), action: 'schedule.delete-month', target: month });
