...
import { enforceLockedForecast, enforceLockedForecastForPublish } from './monthly-forecast.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      if (sesja.role === 'pracownik') {
        ...
        if (!accountId && !name && month) {
          ...
          // podgląd zespołu: pełne imiona i nazwiska z KONT (alias tylko gdy brak konta) + flaga „moja zmiana"
          const konta = (await kv.get('accounts:list')) || [];
          const poId = new Map(konta.map((a) => [a.id, a]));
          const poAlias = new Map();
          konta.forEach((a) => [a.grafikName, ...(a.aliasy || [])].filter(Boolean).forEach((n) => poAlias.set(normalizeName(n), a)));
          const shifts = (m.shifts || []).map(({ accountId: aid, ...rz }) => {
            const konto = (aid && poId.get(aid)) || poAlias.get(normalizeName(rz.name)) || null;
            return { ...rz, name: (konto && konto.name) || rz.name, mine: aid ? aid === sesja.accountId : normalizeName(rz.name) === normalizeName(sesja.grafikName || sesja.name) };
          });
          return res.json({ success: true, shifts, meta: m.meta, opublikowany: !m.niepublikowany });
        }
    ...
    if (req.method === 'POST' && req.query && req.query.action === 'add') {
      ...
      const forecastGuard = await enforceLockedForecast(ym, m.shifts);
      if (!forecastGuard.ok) return res.status(409).json({ success: false, forecastLimit: true, error: forecastGuard.error, violations: forecastGuard.violations, compliance: forecastGuard });
    ...
    if (req.method === 'POST' && req.query && req.query.action === 'publish') {
      ...
      const working = await readMonthEnsure(ym);
      const forecastPublishGuard = await enforceLockedForecastForPublish(ym, working.shifts);
      if (!forecastPublishGuard.ok) return res.status(409).json({ success: false, forecastLimit: true, error: forecastPublishGuard.error, violations: forecastPublishGuard.violations, compliance: forecastPublishGuard });
      if (!(working.shifts || []).length) return res.status(400).json({ success: false, error: 'Ten miesiąc nie ma żadnych zmian.' });
      // Bramka zgodności z prawem pracy: twarde naruszenia blokują publikację (force + uzasadnienie = wyjątek ASM z audytem)
      const kontaC = (await kv.get('accounts:list')) || [];
      const zgodnosc = ocenZgodnosc(working.shifts, kontaC, { from: `${ym}-01`, to: `${ym}-31` });
      const wymus = !!(req.body || {}).force;
      const powodWymuszenia = String((req.body || {}).reason || '').trim();
      if (zgodnosc.summary.block > 0 && !wymus) {
        return res.status(409).json({ success: false, compliance: true, error: `Grafik narusza twarde reguły prawa pracy (${zgodnosc.summary.block}). Popraw zmiany albo opublikuj z uzasadnieniem.`, violations: zgodnosc.violations.filter((v) => v.level === 'block').slice(0, 60), summary: zgodnosc.summary });
      }
      if (zgodnosc.summary.block > 0 && wymus && powodWymuszenia.length < 5) {
        return res.status(400).json({ success: false, compliance: true, error: 'Publikacja z naruszeniami wymaga uzasadnienia (min. 5 znaków).', summary: zgodnosc.summary });
      }
      ...
      await audit({ ...aktor(sesjaPub), action: 'schedule.publish', target: ym, after: { wersjaPub: pub.wersjaPub, zmian: pub.shifts.length, naruszenia: zgodnosc.summary } });
      if (zgodnosc.summary.block > 0) await audit({ ...aktor(sesjaPub), action: 'schedule.publish-forced', target: ym, after: { powod: powodWymuszenia, naruszenia: zgodnosc.summary, kody: [...new Set(zgodnosc.violations.filter((v) => v.level === 'block').map((v) => v.code))] } });
      return res.json({ success: true, month: ym, wersjaPub: pub.wersjaPub, zmian: pub.shifts.length, compliance: zgodnosc.summary });
    }
    ...
    // POST ?action=add-bulk — DOPISANIE wielu zmian do istniejącego grafiku (nie zastępuje miesiąca).
    // Duplikaty (ta sama osoba, data i godziny) są pomijane i raportowane.
    if (req.method === 'POST' && req.query && req.query.action === 'add-bulk') {
      const sesjaB = await requireRole(req, res, ['asm', 'kierownik']);
      if (!sesjaB) return;
      const wejscie = Array.isArray((req.body || {}).shifts) ? req.body.shifts : [];
      if (!wejscie.length) return res.status(400).json({ success: false, error: 'Brak zmian do dopisania.' });
      const konta = await wczytajKonta();
      const mapa = mapaNazw(konta);
      const godziny = (a, b) => { const [h1, m1] = String(a).split(':').map(Number); const [h2, m2] = String(b).split(':').map(Number); let x = h1 * 60 + (m1 || 0), y = h2 * 60 + (m2 || 0); if (y <= x) y += 1440; return (y - x) / 60; };
      const czasOk2 = (t) => /^([01]?\d|2[0-3]):[0-5]\d$/.test(String(t || ''));
      const wgYm = {};
      const bledne = [];
      wejscie.forEach((z, i) => {
        const date = String(z.date || '');
        if (!/^\d{4}-\d{2}-\d{2}$/.test(date) || !z.name || !czasOk2(z.start) || !czasOk2(z.end)) { bledne.push(i + 1); return; }
        (wgYm[date.slice(0, 7)] = wgYm[date.slice(0, 7)] || []).push(z);
      });
      let dodane = 0, pominiete = 0; const nieprzypisane = {};
      const idx = (await kv.get(INDEX_KEY)) || [];
      for (const [ym, arr] of Object.entries(wgYm)) {
        const m = (await kv.get(monthKey(ym))) || { shifts: [], roster: [], meta: { ym } };
        const istnieje = new Set((m.shifts || []).map((x) => `${normalizeName(x.name)}|${x.date}|${x.start}|${x.end}`));
        for (const z of arr) {
          const osoba = String(z.name).trim().toUpperCase();
          const klucz = `${normalizeName(osoba)}|${z.date}|${z.start}|${z.end}`;
          if (istnieje.has(klucz)) { pominiete++; continue; }
          istnieje.add(klucz);
          const idKonta = z.accountId || mapa.get(normalizeName(osoba)) || null;
          if (!idKonta) nieprzypisane[osoba] = (nieprzypisane[osoba] || 0) + 1;
          m.shifts.push({ sid: newSid(), date: z.date, name: osoba, station: String(z.station || 'MANAGER').toUpperCase(), start: z.start, end: z.end, hours: z.hours != null ? Number(z.hours) : godziny(z.start, z.end), dodana: true, ...(idKonta ? { accountId: idKonta } : {}) });
          if (!(m.roster || []).some((r2) => normalizeName(r2) === normalizeName(osoba))) m.roster = [...(m.roster || []), osoba];
          dodane++;
        }
        const forecastGuard = await enforceLockedForecast(ym, m.shifts, konta);
        if (!forecastGuard.ok) return res.status(409).json({ success: false, forecastLimit: true, error: forecastGuard.error, violations: forecastGuard.violations, compliance: forecastGuard });
        await saveMonth(ym, m);
        if (!idx.includes(ym)) idx.push(ym);
      }
      await kv.set(INDEX_KEY, idx.sort());
      await audit({ ...aktor(sesjaB), action: 'schedule.import-add', target: Object.keys(wgYm).join(','), after: { dodane, pominiete, bledne: bledne.length } });
      return res.json({ success: true, dodane, pominiete, bledne, miesiace: Object.keys(wgYm).sort(), nieprzypisane });
    }

    // POST ?action=szkolenie — nadanie roli instruktora i przypisanie ucznia (albo rozpięcie pary).
    // Model: uczeń dostaje rola='training' + partner=INSTRUKTOR; instruktor równoległy wiersz
    // techniczny (station=INSTRUKTOR, rola='instruktor', partner=UCZEŃ) na godziny ucznia.
    if (req.method === 'POST' && req.query && req.query.action === 'szkolenie') {
      const sesjaSzk = await requireRole(req, res, ['asm', 'kierownik']);
      if (!sesjaSzk) return;
      const { date, instruktor, uczen } = req.body || {};
      if (!date || !instruktor || !(instruktor.sid || instruktor.name)) return res.status(400).json({ success: false, error: 'Wskaż dzień i instruktora.' });
      const ym = String(date).slice(0, 7);
      ...
      const techniczny = (x) => x.rola === 'instruktor' || UP(x.station) === 'INSTRUKTOR';
      const iShift = instruktor.sid
        ? (m.shifts || []).find((x) => x.sid === instruktor.sid && !techniczny(x))
        : (m.shifts || []).find((x) => x.date === date && normalizeName(x.name) === normalizeName(instruktor.name) && !techniczny(x));
      if (!iShift) return res.status(404).json({ success: false, error: 'Instruktor nie ma zmiany roboczej tego dnia.' });

      // zdejmij dotychczasową parę tego instruktora w tym dniu
      const stareTech = (m.shifts || []).filter((x) => x.date === date && normalizeName(x.name) === normalizeName(iShift.name) && techniczny(x));
      m.shifts = (m.shifts || []).filter((x) => !stareTech.includes(x));
      m.shifts = m.shifts.map((x) => {
        if (x.date === date && x.rola === 'training' && normalizeName(x.partner || '') === normalizeName(iShift.name)) {
          const { rola: _r, partner: _p, ...czysty } = x; return czysty;
        }
        return x;
      });

      let uShift = null;
      if (uczen && String(uczen).trim()) {
        uShift = (m.shifts || []).find((x) => x.date === date && normalizeName(x.name) === normalizeName(uczen) && !techniczny(x));
        if (!uShift) return res.status(404).json({ success: false, error: `Uczeń ${uczen} nie ma zmiany tego dnia.` });
        if (normalizeName(uShift.name) === normalizeName(iShift.name)) return res.status(400).json({ success: false, error: 'Instruktor nie może szkolić samego siebie.' });
        // zdejmij ewentualnego poprzedniego instruktora ucznia
        const cudzyTech = (m.shifts || []).filter((x) => x.date === date && techniczny(x) && normalizeName(x.partner || '') === normalizeName(uShift.name));
        m.shifts = m.shifts.filter((x) => !cudzyTech.includes(x));
        uShift.rola = 'training';
        uShift.partner = iShift.name;
        m.shifts.push({ sid: newSid(), date, name: iShift.name, station: 'INSTRUKTOR', start: uShift.start, end: uShift.end, hours: uShift.hours != null ? uShift.hours : undefined, rola: 'instruktor', partner: uShift.name, dodana: true, ...(iShift.accountId ? { accountId: iShift.accountId } : {}) });
      }
      const wersja = await saveMonth(ym, m);
      if (uShift) await audit({ ...aktor(sesjaSzk), action: 'schedule.training-pair', target: `${date}`, after: { instruktor: iShift.name, uczen: uShift.name, godziny: `${uShift.start}–${uShift.end}` } });
      else await audit({ ...aktor(sesjaSzk), action: 'schedule.training-unpair', target: `${date}`, before: { instruktor: iShift.name } });
      return res.json({ success: true, instruktor: iShift.name, uczen: uShift ? uShift.name : null, version: wersja });
    }
    ...
    if (req.method === 'PUT') {
      ...
      const forecastGuard = await enforceLockedForecast(ym, data.shifts, konta);
      if (!forecastGuard.ok) return res.status(409).json({ success: false, forecastLimit: true, error: forecastGuard.error, violations: forecastGuard.violations, compliance: forecastGuard });
