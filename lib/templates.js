...
export default async function handler(req, res) {
  ...
  try {
    // szablony: odczyt i użycie dla ASM/kierownika, kasowanie tylko ASM
    const sesja = await requireRole(req, res, req.method === 'DELETE' ? ['asm'] : ['asm', 'kierownik']);
    if (!sesja) return;
    const db = (await kv.get(KEY)) || { list: [] };

    if (req.method === 'GET') return res.json({ success: true, templates: db.list.map((t) => {
      const dniH = [0, 0, 0, 0, 0, 0, 0];
      t.sloty.forEach((sl) => sl.shifts.forEach((sh) => { dniH[sh.dow] += sh.hours; }));
      return { id: t.id, name: t.name, notes: t.notes, createdAt: t.createdAt, fav: !!t.fav, sloty: t.sloty.length, zmian: t.sloty.reduce((a, s2) => a + s2.shifts.length, 0), godzin: t.sloty.reduce((a, s2) => a + s2.shifts.reduce((x, y) => x + y.hours, 0), 0), dniH, otwarte: t.sloty.filter((s2) => !s2.hintAccountId).length };
    }) });
    ...
    // POST ?action=fav {id} — przełącz ulubiony
    if (req.method === 'POST' && req.query && req.query.action === 'fav') {
      const t = db.list.find((x) => x.id === (req.body || {}).id);
      if (!t) return res.status(404).json({ success: false, error: 'Nie znaleziono szablonu' });
      t.fav = !t.fav;
      await kv.set(KEY, db);
      return res.json({ success: true, fav: t.fav });
    }

    // POST ?action=duplicate {id} — kopia szablonu
    if (req.method === 'POST' && req.query && req.query.action === 'duplicate') {
      const t = db.list.find((x) => x.id === (req.body || {}).id);
      if (!t) return res.status(404).json({ success: false, error: 'Nie znaleziono szablonu' });
      const kopia = { ...JSON.parse(JSON.stringify(t)), id: 'tpl' + Date.now(), name: `${t.name} (kopia)`, fav: false, createdAt: new Date().toISOString() };
      db.list.push(kopia);
      await kv.set(KEY, db);
      await audit({ ...aktor(sesja), action: 'template.duplicate', target: t.id, after: { nowy: kopia.id } });
      return res.json({ success: true, template: { id: kopia.id, name: kopia.name } });
    }
    ...
    if (req.method === 'POST' && req.query && req.query.action === 'apply') {
      ...
      for (const slot of t.sloty) {
        ...
        for (const sh of slot.shifts) {
          ...
          dodane.push({ sid: 'sh_' + randomUUID(), date: ymd(d), name: osoba, station: sh.station, start: sh.start, end: sh.end, hours: sh.hours, dodana: true, tpl: t.id, ...(idKonta ? { accountId: idKonta } : {}) });
        }
      }
      ...
      for (const [ym, arr] of Object.entries(wgYm)) {
        ...
        m.version = (m.version || 0) + 1; m.meta = { ...(m.meta || {}), version: m.version };   // DATA-03
        await kv.set(monthKey(ym), m);
        if (!idx.includes(ym)) idx.push(ym);
      }
      await kv.set(INDEX_KEY, idx.sort());
      await audit({ ...aktor(sesja), action: 'template.apply', target: t.id, after: { tydzien: weekStart, zmian: dodane.length } });
