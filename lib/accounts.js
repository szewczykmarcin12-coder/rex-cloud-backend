...
const safe = (a) => ({ id: a.id, name: a.name, grafikName: a.grafikName, aliasy: a.aliasy || [], funkcja: a.funkcja, umowa: a.umowa, stawka: a.stawka, zus: a.zus, instruktor: !!a.instruktor, login: a.login, mustChange: a.mustChange, karta: !!(a.cardTokenHash || a.cardToken), wymiarTygH: a.wymiarTygH || null, maxDobaH: a.maxDobaH || null, stanowiska: a.stanowiska || [] });
// Widok dla kierownika zmiany — bez danych płacowych (macierz uprawnień: R ograniczony)
const safeLimited = (a) => ({ id: a.id, name: a.name, grafikName: a.grafikName, aliasy: a.aliasy || [], funkcja: a.funkcja, instruktor: !!a.instruktor, login: a.login, mustChange: a.mustChange, wymiarTygH: a.wymiarTygH || null, maxDobaH: a.maxDobaH || null, stanowiska: a.stanowiska || [] });
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST' && action === 'reset') {
      ...
      await write(list);
      await audit({ ...aktor(s), action: 'account.reset-password', target: a.login });
    ...
    if (req.method === 'POST' && action === 'card') {
      ...
      await write(list);
      await audit({ ...aktor(s), action: a.cardTokenHash ? 'account.card-set' : 'account.card-clear', target: a.login });
    ...
    if (req.method === 'POST') {
      ...
      if (pin && String(pin).trim() && !/^\d{4,8}$/.test(String(pin).trim())) return res.status(400).json({ success: false, error: 'PIN musi mieć 4–8 cyfr' });   // COR-07
      ...
      const acc = { id: 'u' + Date.now(), name: nazwa, grafikName: (grafikName && String(grafikName).trim()) ? String(grafikName).trim().toUpperCase() : grafikOf(nazwa), aliasy: Array.isArray(aliasy) ? aliasy.map((x) => String(x).trim().toUpperCase()).filter(Boolean) : [], funkcja: funkcja || 'CREW', umowa: rest ? null : (umowa || 'UZ'), stawka: rest ? 0 : (Number(stawka) || 0), zus: rest ? false : ((umowa === 'UOP') ? true : !!zus), instruktor: rest ? false : !!instruktor, login, hasloHash, mustChange,
        wymiarTygH: Number(req.body.wymiarTygH) || null, maxDobaH: Number(req.body.maxDobaH) || null,
        stanowiska: Array.isArray(req.body.stanowiska) ? req.body.stanowiska.map((x) => String(x).trim().toUpperCase()).filter(Boolean) : [] };
      list.push(acc);
      await write(list);
      await audit({ ...aktor(s), action: 'account.create', target: login, after: { name: acc.name, funkcja: acc.funkcja, umowa: acc.umowa } });
    ...
    if (req.method === 'PUT') {
      ...
      if (patch.wymiarTygH !== undefined) a.wymiarTygH = Number(patch.wymiarTygH) || null;     // WFM-04
      if (patch.maxDobaH !== undefined) a.maxDobaH = Number(patch.maxDobaH) || null;
      if (patch.stanowiska !== undefined) a.stanowiska = Array.isArray(patch.stanowiska) ? patch.stanowiska.map((x) => String(x).trim().toUpperCase()).filter(Boolean) : [];
      await write(list);
      await audit({ ...aktor(s), action: 'account.update', target: a.login, after: { ...patch, hasloHash: undefined } });
      return res.json({ success: true, account: safe(a) });
    }
    ...
    if (req.method === 'DELETE') {
      ...
      if (!s) return;
      const list = await read();
      const usuwane = list.find((x) => x.id === id);
      await write(list.filter((x) => x.id !== id));
      if (usuwane) await audit({ ...aktor(s), action: 'account.delete', target: usuwane.login, before: { name: usuwane.name, funkcja: usuwane.funkcja } });
