...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST' && action === 'reset') {
      ...
      await write(list);
      audit({ ...aktor(s), action: 'account.reset-password', target: a.login });
    ...
    if (req.method === 'POST' && action === 'card') {
      ...
      await write(list);
      audit({ ...aktor(s), action: a.cardTokenHash ? 'account.card-set' : 'account.card-clear', target: a.login });
    ...
    if (req.method === 'POST') {
      ...
      list.push(acc);
      await write(list);
      audit({ ...aktor(s), action: 'account.create', target: login, after: { name: acc.name, funkcja: acc.funkcja, umowa: acc.umowa } });
    ...
    if (req.method === 'PUT') {
      ...
      await write(list);
      audit({ ...aktor(s), action: 'account.update', target: a.login, after: { ...patch, hasloHash: undefined } });
      return res.json({ success: true, account: safe(a) });
    }
    ...
    if (req.method === 'DELETE') {
      ...
      if (usuwane) audit({ ...aktor(s), action: 'account.delete', target: usuwane.login, before: { name: usuwane.name, funkcja: usuwane.funkcja } });
