...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') { // create
      const { name, funkcja, umowa, stawka, zus, instruktor, grafikName, aliasy } = req.body || {};
      ...
      const haslo = genPass();
      const acc = { id: 'u' + Date.now(), name: name.trim(), grafikName: (grafikName && String(grafikName).trim()) ? String(grafikName).trim().toUpperCase() : grafikOf(name), aliasy: Array.isArray(aliasy) ? aliasy.map((x) => String(x).trim().toUpperCase()).filter(Boolean) : [], funkcja: funkcja || 'CREW', umowa: umowa || 'UZ', stawka: Number(stawka) || 0, zus: (umowa === 'UOP') ? true : !!zus, instruktor: !!instruktor, login, hasloHash: sha(haslo), mustChange: true };
      list.push(acc);
      await write(list);
