...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') { // create
      ...
      const rest = String(funkcja || '').toUpperCase() === 'REST';   // konto restauracji: bez umowy/stawki, login+PIN podawane wprost
      if (!rest && (!name || name.trim().split(/\s+/).length < 2)) return res.status(400).json({ success: false, error: 'Podaj imię i nazwisko' });
      if (rest && !name) return res.status(400).json({ success: false, error: 'Podaj nazwę konta' });
      const list = await read();
      let login;
      ...
      const acc = { id: 'u' + Date.now(), name: name.trim(), grafikName: (grafikName && String(grafikName).trim()) ? String(grafikName).trim().toUpperCase() : grafikOf(name), aliasy: Array.isArray(aliasy) ? aliasy.map((x) => String(x).trim().toUpperCase()).filter(Boolean) : [], funkcja: funkcja || 'CREW', umowa: rest ? null : (umowa || 'UZ'), stawka: rest ? 0 : (Number(stawka) || 0), zus: rest ? false : ((umowa === 'UOP') ? true : !!zus), instruktor: rest ? false : !!instruktor, login, hasloHash, mustChange };
      list.push(acc);
      await write(list);
