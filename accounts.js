...
const safe = (a) => ({ id: a.id, name: a.name, grafikName: a.grafikName, funkcja: a.funkcja, umowa: a.umowa, stawka: a.stawka, zus: a.zus, login: a.login, mustChange: a.mustChange });
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') { // create
      const { name, funkcja, umowa, stawka, zus } = req.body || {};
      ...
      const haslo = genPass();
      const acc = { id: 'u' + Date.now(), name: name.trim(), grafikName: grafikOf(name), funkcja: funkcja || 'CREW', umowa: umowa || 'UZ', stawka: Number(stawka) || 0, zus: (umowa === 'UOP') ? true : !!zus, login, hasloHash: sha(haslo), mustChange: true };
      list.push(acc);
      await write(list);
